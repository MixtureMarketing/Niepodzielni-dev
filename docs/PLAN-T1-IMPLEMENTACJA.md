...
6. **Carbon Fields Block** w `cpt/21-carbon-fields.php`:
   ```php
   use Carbon_Fields\Block;
   Block::make('np/donate', 'Blok Darowizny')
       ->add_fields([
           Field::make('select', 'donate_mode', 'Tryb')
               ->set_options(['all' => 'Wszystkie 3', 'pit' => 'Tylko PIT', 'onetime' => 'Tylko jednorazowy', 'monthly' => 'Tylko subskrypcja']),
           Field::make('text', 'preset_amounts', 'Sugerowane kwoty (zł, oddzielone przecinkami)')->set_default_value('50, 100, 200'),
           Field::make('rich_text', 'cta_text', 'Tekst nad przyciskami'),
       ])
       ->set_render_callback(fn($block, $content, $is_preview) => 
           print view('blocks.donate', compact('block', 'is_preview'))->render()
       );
   ```
   Blade: `web/app/themes/niepodzielni-theme/resources/views/blocks/donate.blade.php`.
...
8. **Strona `/wesprzyj/`**:
   - WP Admin → Strony → Dodaj nową „Wesprzyj nas" → slug `wesprzyj`,
   - W edytorze: dodaj blok `np/donate` z trybem `all`.
...
| `web/app/mu-plugins/niepodzielni-core/cpt/21-carbon-fields.php` | Rozszerzenie o Block np/donate |
| `web/app/themes/niepodzielni-theme/resources/views/blocks/donate.blade.php` | Nowy partial bloku |
...
4. WP Admin → Strony → utwórz „Wesprzyj nas" → blok `np/donate` mode=all.
5. Frontend `/wesprzyj/`:
